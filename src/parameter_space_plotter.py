...
Usage:
    python parameter_space_plotter.py <config.json> <experiment_number>
"""
...
def plot_parameter_space(config: Dict[str, Any], log_entry: Dict[str, Any],
                         experiment_num: int, output_path: str) -> None:
    ...
    # Set labels
    ax.set_xlabel(param_vars[0], fontsize=12)
    ax.set_ylabel(param_vars[1], fontsize=12)
...
def main():
    """Main entry point."""
    if len(sys.argv) != 3:
        print("Usage: python parameter_space_plotter.py <config.json> <experiment_number>")
        sys.exit(1)
    ...
    # Create the plot
    plot_parameter_space(config, log_entry, experiment_num, output_path)
